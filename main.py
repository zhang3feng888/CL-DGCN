...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="batch size for training and validation (default: 25)",
)
parser.add_argument(
    "--epochs", type=int, default=50, help="epochs for training  (default: 50)"
)
parser.add_argument(
    "--DEEP_num_layers", type=int, default=3, help="Number of GNN layers (default: 3)"
)
parser.add_argument("--window", type=int, default=144, help="window length")
parser.add_argument(
    "--sensorsfilepath",
    type=str,
    default="./data/METRLA/graph_sensor_ids.txt",
    help="sensors file path",
)
parser.add_argument(
    "--disfilepath",
    type=str,
    default="./data/METRLA/distances_la_2012.csv",
    help="distance file path",
)
parser.add_argument(
    "--tsfilepath", type=str, default="./data/METRLA/metr-la.h5", help="ts file path"
)
...
G = dgl.from_scipy(sp_mat=sp_mx, eweight_name="feat")
...
loss = nn.MSELoss()
G = G.to(device)
model = STGCN_WAVE(blocks, n_his, n_route, G, drop_prob, DEEP_num_layers, device, args.control_str).to(device)
...
best_model = STGCN_WAVE(
    blocks, n_his, n_route, G, drop_prob, DEEP_num_layers, device, args.control_str
).to(device)
